...
function directory_manual_entries(array $categories): array
{
    ...
    return [
        ...
        [
            ...
            // Notes: only extras not already covered by Links / Parameters (facts) / Verification tools.
            'notes' => [
                'en' => '<p>Custodial exchange focused on privacy-oriented fixed routes, Monero exits, shielded Zcash, and same-asset reserve swaps. Operator policy also claims no AML-score surcharge and that funds are not held pending identity or source-of-funds documents (policy only — not an audit).</p><p>Route groups (operator): BTC and USDT TRON private reserve swaps; 12 routes for buying Monero privately; 10 routes for spending Monero privately; routes into BTC and ETH; routes into USDT and USDC stablecoins.</p><p>Before the customer sends funds, the exact fee, rate, minimum, maximum, required confirmations, and final receive amount are displayed and fixed. Payouts come from separate reserve liquidity. Each order gets a Letter of Guarantee signed before payment with address <code>bc1q6etkarmarvej0874ghw396eacejlsy5ygew5yk</code> — verify on the <a href="https://betkarma.art/en/letter-of-guarantee" target="_blank" rel="noopener noreferrer">official key page</a>, with the tool below, or the <a href="https://grggoo.github.io/betkarma-verify/letter.html?lang=en" target="_blank" rel="noopener noreferrer">open-source verifier</a> before paying.</p>',
                'ru' => '<p>Кастодиальный обменник с упором на privacy-маршруты, выходы в Monero, shielded Zcash и same-asset свопы из резерва. Оператор также заявляет отсутствие надбавки по AML-score и что средства не удерживаются до документов о личности/источнике средств (только политика — не аудит).</p><p>Группы маршрутов (по оператору): приватные резервные свопы BTC и USDT TRON; 12 маршрутов покупки Monero; 10 маршрутов траты Monero; маршруты в BTC и ETH; маршруты в USDT и USDC.</p><p>До отправки средств точная комиссия, курс, минимум, максимум, число подтверждений и сумма к получению показываются и фиксируются. Выплаты из отдельной резервной ликвидности. Каждая заявка получает Letter of Guarantee до оплаты с подписью адреса <code>bc1q6etkarmarvej0874ghw396eacejlsy5ygew5yk</code> — проверяйте на <a href="https://betkarma.art/ru/letter-of-guarantee" target="_blank" rel="noopener noreferrer">официальной странице ключа</a>, инструментом ниже или в <a href="https://grggoo.github.io/betkarma-verify/letter.html?lang=ru" target="_blank" rel="noopener noreferrer">открытом верификаторе</a> до оплаты.</p>',
            ],
            'config' => [
                'en' => '',
                'ru' => '',
